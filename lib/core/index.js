...
export const getAST = (filePath: string): gASTNode | null => {
  ...
  /* There is a bug in gonzales.parse. Parsing `.scss` files which include mixins (e.g.
   * `@include mixins.header;`) fails. So, we replace those kind of statements with
   * `@include mixins_header`. */
  const fileContentString = fileContent
    .toString()
    .replaceAll(/@include ([a-z]+)\.([a-z]+)/g, '@include $1_$2');
